...
{    
    ...
    // Make sure the model has the uid property.
    // Otherwise, instanciate a new entity.
    //
    BOOL isSyncable = (class_getProperty(self, [@"uid" UTF8String]) != NULL);
    
    if ([LBObjectManager isString: newId] && isSyncable) {
        NSArray *result = [self fetch: @"uid == %@", newId];
        if (result != nil && result.count > 0) {
            ...
            [entity setValue: newId 
                      forKey: @"uid"];
            NSLog(@"OHData : Insert new %@ with uid = \"%@\"", [LBObjectManager getClassNameFromObject: self], newId);
        }
